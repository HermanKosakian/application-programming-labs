...
def d_image(dir_name: str,keyword: str) -> None:
    """
    Функция выполняет установку изображения с помощью GoogleImageCrawler, перед этим создает директорий(если необходимо) и очищает директорий(если необходимо)
